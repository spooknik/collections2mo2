...
def build_items_with_separators(
    ...
    for idx in order_idx:
        ...
        items.append(
            ProfileItem(kind="mod", name=entries[idx]["folder"], entry_idx=idx, phase=phase)
        )
        run_sep = sep
        run_phase = phase
        run_started = True
...
GAME_INIS: dict[str, tuple[str, list[str], str]] = {
    ...
    "Starfield": (
        "Starfield",
        ["Starfield.ini", "StarfieldPrefs.ini", "StarfieldCustom.ini"],
        "Starfield_Default.ini",
    ),
}


def seed_profile_inis(
    profile_dir: Path, game_name: str, game_path: str
...
def merge_ini_values(
    ...
    for key, value in values.items():
        ...
        hits = [
            ...
            for h in (_find_key(lines, start, end, key),)
            if h is not None
        ]
        if hits:
...
def merge_ini_tweak(
    ...
    `c2mo2 remove` knows exactly which INI keys this layer is responsible for and what value
...
def apply_collection_ini_tweaks(
    ...
    spec = GAME_INIS.get(game_name)
    known = {
        n.lower()[:-4]: n for n in (spec[1] if spec else [])
    }  # "skyrimprefs" -> "SkyrimPrefs.ini"
    notes: list[str] = []
...
def _parse_resolution_arg(value: str) -> str:
    ...
    raise argparse.ArgumentTypeError(
        f"must be 'auto', 'keep', or WxH e.g. 3440x1440 (got {value!r})"
    )
...
DISPLAY_OVERRIDE_MOD_NAME = "c2mo2 Display Settings"
DISPLAY_OVERRIDE_MARKER = "c2mo2-display"
# Pre-rename names. An instance built before the project became `collections2mo2`
# still has the mod folder and marker below until `create.migrate_legacy_instance`
# renames them, so `_override_is_active` accepts both pairs.
LEGACY_DISPLAY_OVERRIDE_MOD_NAME = "c2wj Display Settings"
LEGACY_DISPLAY_OVERRIDE_MARKER = "c2wj-display"
    ...
    for key, value in wanted.items():
        ...
        if hits:
            for i in hits:
                lines[i] = f"{key}={value}"
            continue
        start = next((i for i, ln in enumerate(lines) if ln.strip().lower() == "[render]"), None)
        if start is None:
...
gameName={game_name}
comments=owner: user
notes=Generated by c2mo2 ({marker}) to keep your --resolution/--window/--vsync choice in \
...
def _override_is_active(mods_dir: Path, led: ledger_mod.Ledger) -> bool:
    """Whether our generated display-override mod is ours and still has its file on disk.
    ...
    this pass are finalised). The pre-rename name/marker pair counts too, so an instance
    that has not yet been migrated still recognises its own override mod.
    """
    return _active_override_name(mods_dir, led) is not None


def _active_override_name(mods_dir: Path, led: ledger_mod.Ledger) -> str | None:
    """The mod folder our display override lives in, or `None` if there isn't one.

    Tries the current name/marker pair first, then the pre-rename one, so an instance
    that has not yet been through `create.migrate_legacy_instance` still finds it.
    """
    mods = led.data.get("mods", {})
    for name, marker in (
        (DISPLAY_OVERRIDE_MOD_NAME, DISPLAY_OVERRIDE_MARKER),
        (LEGACY_DISPLAY_OVERRIDE_MOD_NAME, LEGACY_DISPLAY_OVERRIDE_MARKER),
    ):
        record = mods.get(name) or {}
        if record.get("generated_by") != marker:
            continue
        if (mods_dir / name / Path(DISPLAY_TWEAKS_RELPATH)).is_file():
            return name
    return None
...
def apply_sse_display_tweaks_override(
    ...
    *,
    ...
    winning = find_winning_file(
        modlist_path,
        mods_dir,
        DISPLAY_TWEAKS_RELPATH,
        skip={DISPLAY_OVERRIDE_MOD_NAME, LEGACY_DISPLAY_OVERRIDE_MOD_NAME},
    )
    if winning is None:
        return []
    ...
    if choice.is_keep:
        active = _active_override_name(mods_dir, led)
        if active is not None:
            override_text = (mods_dir / active / Path(DISPLAY_TWEAKS_RELPATH)).read_text(
                encoding="utf-8-sig"
            )
...
# -- Layered instances --------------------------------------------------------------
#
# An instance is not one collection forever. `c2mo2 add` layers an add-on collection on
...
#   tool mods     folders `c2mo2 tools install` put in mods/ for a catalogue tool's
...
#                 replaced so `c2mo2 remove` can put exactly that back.
...
def splice_preserved(new_names: list[str], old_names: list[str], preserved: list[str]) -> list[str]:
...
def update_mo2_ini(
    ...
    An instance that has been opened in MO2 (or had `c2mo2 tools install` run against it)
...
def render_instance(
    ...
    *,
    ...
    #    own mods. `c2mo2 tools install` records these in the ledger under `tool:<id>`
    ...
    tool_folders = sorted(
        name
        for name in on_disk
        if name not in managed_set and any(o.startswith("tool:") for o in led.owners_of(name))
    )
    ...
    user_folders = sorted(
        name
        for name in on_disk
        if name not in managed_set and name not in known_separators and name not in led.data["mods"]
    )
    ...
    def _merge_plugin_file(name: str, render) -> list[tuple[str, bool]]:
        ...
        result = [(n, enabled_by_name.get(n, old_markers.get(n, "*") == "*")) for n in merged]
    ...
    path = Path(report_out) if report_out else instance_dir / "c2mo2" / "profile-report.json"
...
def cmd_profile(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    ...
    report_path = (
        ...
        else mo2_dir / "profile-report.json"
    )
...
def cmd_profile_instance(args: argparse.Namespace, reporter: Reporter | None = None) -> int:
    ...
    A thin `render_instance` wrapper: unlike `c2mo2 profile` (which builds a single-layer
    ...
    *existing* instance -- exactly what `create`/`add`/`remove` do -- so it can be used
    ...
    """
    from . import create as create_mod  # local: `create` imports this module

    rep = get_reporter(reporter)
    instance_dir = Path(args.instance).expanduser().resolve()
    create_mod.migrate_legacy_instance(create_mod.Paths(instance_dir), rep)
...
def add_instance_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--instance", required=True, help="instance directory (has c2mo2-instance.json)")
    p.add_argument(
        "--resolution",
        type=_parse_resolution_arg,
        default="keep",
    ...
    p.add_argument(
        ...
        "generated override mod (if c2mo2 still owns it), then re-render",
    )
